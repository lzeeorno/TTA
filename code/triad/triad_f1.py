...
class DELTA(nn.Module):
    ...
    def __init__(
        ...
        # H̄: safe entropy threshold — set at 0.2*ln(C) to center the sigmoid
        # within the actual batch entropy operating range (~1.0-1.5 for ImageNet).
        # Previous 0.4*ln(C)=2.76 was too high, causing α_t to stay near 1.3
        # (sigmoid always in low tail), making D³LR effectively a static decay.
        self.d3lr_h_bar = 0.4 * math.log(num_classes) if self._bn_standard_mode else 0.2 * math.log(num_classes)
        self.base_lr = lr
        self._current_alpha = d3lr_alpha_max  # initial alpha (will be updated dynamically)
        self._effective_d3lr_alpha_max = min(d3lr_alpha_max, 6.0) if self._bn_standard_mode else d3lr_alpha_max
        ...
        self.use_aux_consistency = gate_cfg.get('use_aux_consistency', True)
        ...
        if optimizer is None:
            ...
            if self.use_d3lr:
                param_groups = self._build_d3lr_param_groups(lr)
            ...
            if optimizer_name_l == 'adam':
                base_optimizer = optim.Adam(
                    optimizer_params, lr=lr, weight_decay=self.optim_weight_decay
                )
                ...
                base_optimizer = optim.SGD(
                    optimizer_params, lr=lr, momentum=self.optim_momentum,
    ...
    def _forward_and_adapt(self, x):
        ...
        if not self.no_gate:
            reliable_ratio = (entropys < self.e_margin).float().mean().item()
        ...
        reliable_indices = torch.where(entropys < self.e_margin)[0]

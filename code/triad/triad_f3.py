...
class DELTA(nn.Module):
    ...
    def __init__(
        ...
        self.vit_last_effective_d_margin = 0.0
    ...
    def _forward_and_adapt(self, x):
        ...
        effective_d_margin = self.d_margin
        if self._vit_continual_mode:
            effective_d_margin = 0.15 + 0.05 * max(
                0.0, min(1.0, self.vit_last_high_freq_mean)
            )
            self.vit_last_effective_d_margin = float(effective_d_margin)
        ...
        if use_anti_redundancy:
            ...
            filter_ids_2 = torch.where(torch.abs(cosine_similarities) < effective_d_margin)[0]
    ...
    def get_stats(self):
        ...
        if self._vit_mode:
            ...
            stats['vit_last_effective_d_margin'] = self.vit_last_effective_d_margin
